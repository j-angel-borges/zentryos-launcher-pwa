...
export const ZentryEntertainmentHubScreen: React.FC<Props> = ({ onBack, onNavigate, isDark }) => {
  ...
  // 4 Official Floating Liquid Glass Portals (YouTube, TikTok, Instagram, YouTube Music) - ZERO TEXT
  const apps = [
    {
      id: 'zentry_tube' as ScreenId,
      voiceText: '¡YouTube!',
      glowColor: 'shadow-[0_20px_50px_-10px_rgba(239,68,68,0.55)] hover:shadow-[0_25px_60px_-10px_rgba(239,68,68,0.8)]',
      borderColor: 'border-red-400/50 hover:border-red-300',
      floatAnim: 'animate-float-1',
      renderLogo: () => (
        <svg viewBox="0 0 24 24" className="w-18 h-18 sm:w-22 sm:h-22 drop-shadow-xl" fill="currentColor">
          <path
            fill="#FF0000"
    ...
    },
    {
      ...
      voiceText: '¡TikTok!',
      glowColor: 'shadow-[0_20px_50px_-10px_rgba(0,242,254,0.45)] hover:shadow-[0_25px_60px_-10px_rgba(254,44,85,0.75)]',
      borderColor: 'border-cyan-400/50 hover:border-pink-400',
      floatAnim: 'animate-float-2',
      renderLogo: () => (
        <svg viewBox="0 0 48 48" className="w-18 h-18 sm:w-22 sm:h-22 drop-shadow-2xl">
    ...
    },
    {
      ...
      voiceText: '¡Instagram!',
      glowColor: 'shadow-[0_20px_50px_-10px_rgba(225,48,108,0.55)] hover:shadow-[0_25px_60px_-10px_rgba(245,96,64,0.8)]',
      borderColor: 'border-pink-400/50 hover:border-amber-300',
      floatAnim: 'animate-float-3',
      renderLogo: () => (
        <svg viewBox="0 0 48 48" className="w-18 h-18 sm:w-22 sm:h-22 drop-shadow-2xl">
          <defs>
            <radialGradient id="ig-lens-grad-hub" cx="20%" cy="110%" r="140%">
          ...
          <rect width="48" height="48" rx="14" fill="url(#ig-lens-grad-hub)" />
    ...
    },
    {
      ...
      voiceText: '¡YouTube Music!',
      glowColor: 'shadow-[0_20px_50px_-10px_rgba(239,68,68,0.55)] hover:shadow-[0_25px_60px_-10px_rgba(168,85,247,0.75)]',
      borderColor: 'border-red-400/50 hover:border-purple-400',
      floatAnim: 'animate-float-4',
      renderLogo: () => (
        <svg viewBox="0 0 48 48" className="w-18 h-18 sm:w-22 sm:h-22 drop-shadow-2xl">
          <circle cx="24" cy="24" r="24" fill="#FF0000" />
          <circle cx="24" cy="24" r="14" fill="none" stroke="#FFFFFF" strokeWidth="3" />
          <path fill="#FFFFFF" d="M21 16v16l12-8z" />
        </svg>
      )
    }
  ];
      ...
      <div className={(isDark ? 'zentry-glass-dark ' : 'zentry-glass-light ') + 'flex-1 rounded-[36px] p-4 md:p-6 flex flex-col justify-between overflow-hidden shadow-2xl relative'}>
        ...
        <div className="w-full flex items-center justify-between z-30 pb-2">
          ...
          <button
            onClick={() => {
              sounds.playTap();
              voiceService.speakFeedback('¡Toca para entrar a YouTube, TikTok, Instagram o YouTube Music!');
            }}
        ...
        {/* Center: 4 Floating Liquid Glass Brand Logo Cards (ZERO TEXT) */}
        <div className="flex-1 w-full max-w-2xl mx-auto flex items-center justify-center py-2 z-30">
          <div className="grid grid-cols-2 gap-4 sm:gap-6 md:gap-8 w-full max-w-[500px] items-center justify-items-center">
            {apps.map((app) => {
              ...
              return (
                <div
                  ...
                  className={`w-full max-w-[210px] aspect-square rounded-[34px] sm:rounded-[42px] backdrop-blur-2xl border-2 ${app.borderColor} ${app.glowColor} ${app.floatAnim} ${isTapped ? 'animate-pop-scale' : ''} flex items-center justify-center cursor-pointer zentry-press transition-all duration-300 group hover:scale-108 active:scale-95 relative overflow-hidden`}
                  style={{
                    background: isDark 
                ...
                >
                  ...
                  <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/40 via-white/10 to-transparent pointer-events-none rounded-t-[34px] sm:rounded-t-[42px]" />
                  
                  {/* Subtle Inner Refraction Rim */}
                  <div className="absolute inset-0 rounded-[34px] sm:rounded-[42px] border border-white/25 pointer-events-none" />
