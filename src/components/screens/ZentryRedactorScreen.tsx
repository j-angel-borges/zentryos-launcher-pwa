...
import { Edit3, Sparkles, Check, Clock, FileText, Send, ArrowLeft, Eye, ChevronRight, Trash2, Pencil, X, Save, Share2, Download, ExternalLink } from 'lucide-react';
...
export const ZentryRedactorScreen: React.FC<Props> = ({ onBack, isDark }) => {
  ...
  const handleExportToGoogleDocs = () => {
    if (!activeDraft) return;
    sounds.playSuccess();
    navigator.clipboard.writeText(activeDraft.documentContent);
    window.open('https://docs.google.com/document/create', '_blank');
    alert('Texto copiado al portapapeles. Creando nuevo Google Doc para que lo pegues con Ctrl+V.');
  ...
  const handleDownloadFile = () => {
    if (!activeDraft) return;
    sounds.playSuccess();
    const blob = new Blob([activeDraft.documentContent], { type: 'text/markdown;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `${activeDraft.title.replace(/\s+/g, '_')}.md`;
    link.click();
            ...
            <div className="flex items-center justify-between py-1 border-b border-white/10 gap-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0 relative">
                ...
                <button
                  ...
                  title="Exportar a Google Docs o Archivo"
                  className="p-1.5 rounded-full bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 cursor-pointer zentry-press flex items-center gap-1 text-[11px] font-semibold"
                >
                ...
                {showExportMenu && (
                  <div className="absolute right-0 top-8 w-56 rounded-[18px] bg-slate-900 border border-white/20 shadow-2xl p-2 z-50 space-y-1 animate-in fade-in">
                    <button
                      onClick={handleExportToGoogleDocs}
                      className="w-full px-3 py-2 rounded-xl text-left text-xs font-semibold text-white hover:bg-blue-600/30 flex items-center gap-2"
                    >
                    ...
                    <button
                      ...
                      className="w-full px-3 py-2 rounded-xl text-left text-xs font-semibold text-white hover:bg-emerald-600/30 flex items-center gap-2"
                    >
                ...
                {viewMode === 'editor' ? (
                  ...
                  >
                    ...
                    <span>{viewMode === 'chat' ? 'Documento' : 'Chat'}</span>
                  </button>
                )}
              </div>
            </div>
